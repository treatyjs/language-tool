...
export function getAngularHtmlLanguageModule(ts: typeof import('typescript')): LanguagePlugin<AngularHtmlVirtualCode> {
	return {
		createVirtualCode(fileId, languageId, snapshot) {
			if (languageId === 'html') {
				return new AngularHtmlVirtualCode(ts, fileId, snapshot);
			}
		},
...
export class AngularHtmlVirtualCode implements VirtualCode {
	id = 'root';
	languageId = 'html';
	...
	public update(snapshot: ts.IScriptSnapshot) {
		...
		this.embeddedCodes[0].snapshot = this.ts.ScriptSnapshot.fromString(generated.codegen.text);
		this.embeddedCodes[0].mappings = generated.codegen.mappings;
		this.parsed = generated.parsed;
	}
...
function generate(
	...
	function addInterpolationFragment(start: number, end: number) {
		...
		let full = '';
		walkInterpolationFragment(ts, code, ast, (fragment, offset) => {
			full += fragment;
			if (offset !== undefined) {
